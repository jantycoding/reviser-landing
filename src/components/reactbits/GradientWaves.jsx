...
/**
 * GradientWaves (react-bits, вариант JS + CSS).
 *
 * Отличия от исходника — две правки под наш проект:
 *   1. Уважение к prefers-reduced-motion: при включённой настройке рисуем один
 *      статичный кадр и не запускаем rAF-цикл.
 *   2. Отказ от WebGL2 не роняет страницу: если контекст не создался,
 *      компонент просто ничего не рисует, а фон остаётся за ним.
 * Цикл и так останавливается, когда блок уходит из вьюпорта или вкладка
 * уходит в фон — это есть в оригинале.
 */
const GradientWaves = ({
  horizonColor = '#5227FF',
  waveColor = '#FF9FFC',
  ...
  useEffect(() => {
    ...
    /* Полноэкранный шейдер — самый дорогой элемент страницы, а основной
       вход у нас через встроенный браузер инстаграма на телефоне. Поэтому на
       узких экранах рисуем ОДИН кадр и цикл не запускаем: картинка та же,
       расход батареи и кадрового бюджета нулевой. */
    const narrow = window.matchMedia?.('(max-width: 767px)').matches;
    const reduced =
      window.matchMedia?.('(prefers-reduced-motion: reduce)').matches || (narrow && !animateOnNarrow);
    ...
    try {
      ...
      return; // нет WebGL2 — просто не рисуем, страница живёт дальше
    }
    ...
    if (reduced) {
      // один статичный кадр — и больше никакой работы для GPU
