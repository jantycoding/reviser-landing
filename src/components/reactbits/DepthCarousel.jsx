...
/**
 * DepthCarousel (react-bits, вариант JS + CSS), 26.08.2026 по запросу владельца.
 *
 * Стопка уходит в глубину: передний слайд в фокусе, следующие отодвинуты по
 * оси Z, развёрнуты ребром и слегка размыты. Листается колесом, перетаскиванием,
 * стрелками и с клавиатуры. Настройки на месте вызова скопированы из примера
 * библиотеки дословно.
 *
 * ДВЕ ПРАВКИ ОТНОСИТЕЛЬНО ИСХОДНИКА.
 *
 * 1. Вместо библиотеки анимаций — свой твинер (lib/tween.js). Из неё здесь
 *    работала ровно одна функция: прогнать одно число от текущей позиции к
 *    целевой по кривой power3.out. Ради этого в бандл уезжало около 90 КБ, а
 *    установка пакета на машине владельца не прошла и заблокировала сборку
 *    целиком. Кривая скопирована формулой — движение то же.
 *
 * 2. Слайд может быть не картинкой.
 * Оригинал жёстко рисует <img src={item.image}> — он написан под галереи. У нас
 * в карусели четыре текстовых карточки «что получаете на выходе», картинок для
 * них нет и не будет: показывать вместо них заглушки с picsum значило бы
 * поставить на страницу декорации вместо содержания. Поэтому элемент списка
 * теперь может нести поле `content` с готовой разметкой, и тогда слайд рисует
 * её. Ветка с картинкой сохранена нетронутой — если завтра появятся развороты
 * настоящего отчёта, они лягут сюда без единой правки компонента.
 */
...
const DepthCarousel = ({
  ...
  return (
    <div
      ...
      aria-label="Что получаете на выходе"
      tabIndex={0}
    ...
    >
      ...
      {showIndicators && count > 1 && (
        <div className="depth-carousel__dots" role="tablist" aria-label="Карточки">
          {data.map((_, i) => (
            <button
              key={i}
              type="button"
